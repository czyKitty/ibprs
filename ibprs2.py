...
# Convert back
def convertBack(result,outText):
    result[0] = result[0][1:-1]
    newSeries = []
    for i in range(len(result[0])//2):
        newSeries.append((result[0][2*i]+1)//2)
    outText.write("Final Result: "+" ".join(str(x) for x in newSeries)+"    Reversal Distance:"+str(result[1])+"\n")
...
def main():
    ...
    for x in series:
        if x > 0:
            newSeries.extend((2*abs(x)-1,2*abs(x)))
        else:
            newSeries.extend((2*abs(x),2*abs(x)-1))
    # Add begin and end
    newSeries = [0] + newSeries + [len(newSeries) + 1]
